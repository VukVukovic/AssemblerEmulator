...
const set<string> Instruction::wordInstructions = {"call", "jmp",  "jeq", "jne",
                                                   "jgt",  "push", "pop"};
...
bool Instruction::validAdressing() const {
  if (numOfOperands() == 2) {
    if (type == "cmp" || type == "test")
      return true;
    if (type == "xchg" &&
        (operands[0]->getType() == IMMED || operands[1]->getType() == IMMED))
      return false;
    if (operands[1]->getType() == IMMED)
      return false;
  } else if (numOfOperands() == 1 && type == "pop" &&
             operands[0]->getType() == IMMED) {
    return false;
  }
  return true;
}
    ...
    if (size != operandSize && operandSize != 0) {
      if (size == 0)
        ...
        throw AssemblerException(
            "Size of instruction or operands sizes are not matched");
    }
  }
