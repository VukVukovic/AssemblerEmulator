...
void EquTable::resolveSymbols(SymbolTable &symbolTable) {
  for (const auto &equRow : equTable) {
    if (!isResolved(equRow.first) &&
        equTable[equRow.first].canBeResolved(symbolTable)) {
  ...
  if (problem)
    throw AssemblerException("Symbols cannot be resovled: " + problematic +
                             " (undefined or circular reference)");
}

void EquTable::addEqu(const string &equSymbol, int value,
...
void EquTable::printEquTable(ostream &out) const {
  for (auto e : equTable) {
    out << e.first << " = ";
    for (auto s : e.second.depend)
      out << (s.first > 0 ? '+' : '-') << s.second << " ";
    out << endl;
  }
}
...
void EquTable::EQUEntry::resolve(SymbolTable &symbolTable, EquTable &equTable) {
  ...
  for (const auto &p : depend) {
    if (symbolTable.getType(p.second) == EXT && p.first == -1)
      throw AssemblerException("Classification index problem with symbol " +
                               symbol + "; - " + p.second + " is not allowed");

    int index = p.first;
    if (symbolTable.getType(p.second) == EXT)
      index = 1;
    else if (symbolTable.getType(p.second) == ABS)
      index = 0;
  ...
  string reference;
  for (const auto &p : depend) {
    if (classificationIndex[symbolTable.getLocalClassification(p.second)] ==
        1) {
  ...
  switch (type) {
  case ABS:
    symbolTable.defineAbsoluteSymbol(symbol, value + valueAdd);
    break;
  case EXT:
    symbolTable.defineExternSymbol(symbol, value + valueAdd, reference);
    break;
  case REL:
    symbolTable.defineRelocatableSymbol(symbol, value + valueAdd, reference);
    break;
  }
  ...
  if (equTable.waiting.find(symbol) != equTable.waiting.end()) {
    for (const string &w : equTable.waiting.at(symbol)) {
      if (!equTable.isResolved(w) &&
          equTable.equTable.at(w).canBeResolved(symbolTable))
