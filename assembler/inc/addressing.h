...
protected:
  char operandDescr() const override {
    return (char)((0x1 << 5) + (reg.getEncoding() << 1) + reg.getLH());
  }
...
protected:
  char operandDescr() const override {
    return (char)((0x2 << 5) + (reg.getEncoding() << 1));
  }
...
protected:
  char operandDescr() const override {
    return (char)((0x3 << 5) + (reg.getEncoding() << 1));
  }
...
public:
  RegisterIndDispSymbol(Register reg, const string &symbol)
      : reg(reg), symbol(symbol) {}
  AddressingType getType() const override { return REGIND; }
  int getBytesSize(int operSize) const override { return 1 + 2; }
  Encoding getEncoding(int size) const override;
};
...
public:
  ...
  int getBytesSize(int operSize) const override { return 1 + 2; }
  void setPCRelOffset(int pcRelOffset) override {
    this->pcRelOffset = pcRelOffset - 1;
  } // sub 1 for descr
  Encoding getEncoding(int size) const override;
};
